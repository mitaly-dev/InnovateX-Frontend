...
const ReportProduct = ({ product, refetch }) => {
  ...
  const reportProductDelete = (title) => {
    ...
    if (confirm) {
      fetch(`${process.env.REACT_APP_PORT}/reportedProduct?title=${title}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
    ...
    <li className="flex flex-col py-6 sm:flex-row sm:justify-between">
      <div className="flex w-full space-x-2 sm:space-x-4">
        ...
        <div className="flex flex-col justify-between w-full pb-4">
          <div className="sm:flex justify-between w-full pb-2 space-x-2 font-roboto">
          ...
          <div className="sm:flex text-sm divide-x font-roboto">
            <button
