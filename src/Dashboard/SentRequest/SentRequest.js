...
const SentRequest = () => {
  ...
  const sendRequestToAdmin = () => {
    ...
    fetch(`${process.env.REACT_APP_PORT}/users/sendRequest?email=${email}`, {
      method: "PUT",
      headers: {
  ...
  const cancelRequestToAdmin = () => {
    const email = user?.email;
    fetch(`${process.env.REACT_APP_PORT}/users/cancelRequest?email=${email}`, {
      method: "PATCH",
      headers: {
        "content-type": "application/json",
      },
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_PORT}/verifyRole?email=${user?.email}`)
      .then((res) => res.json())
